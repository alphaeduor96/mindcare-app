...
alter table public.psicologo_configuracion
  add column if not exists horario_inicio time not null default '08:00',
  add column if not exists horario_cierre time not null default '20:00',
...
grant select, insert, update, delete on public.psicologo_configuracion to authenticated;

drop policy if exists "psicologo_configuracion_owner_select" on public.psicologo_configuracion;
create policy "psicologo_configuracion_owner_select"
on public.psicologo_configuracion for select
to authenticated
using (
  public.is_admin()
...
drop policy if exists "psicologo_configuracion_owner_insert" on public.psicologo_configuracion;
create policy "psicologo_configuracion_owner_insert"
on public.psicologo_configuracion for insert
to authenticated
with check (
  public.is_admin()
  or public.is_psychologist_profile_owner(psicologo_id)
...
drop policy if exists "psicologo_configuracion_owner_update" on public.psicologo_configuracion;
create policy "psicologo_configuracion_owner_update"
on public.psicologo_configuracion for update
to authenticated
using (
  public.is_admin()
  or public.is_psychologist_profile_owner(psicologo_id)
